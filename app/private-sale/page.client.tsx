...
import { presaleContract, tokenContract } from '@/lib/contracts';
...
export default function SLDPrivateSalePage() {
  ...
  // BNB balance for wallet
  const { data: bnbBalData } = useBalance({
  address,
  query: {
    refetchInterval: 8000, // refresh balance every 8s
  },
});
  ...
  const { data: rateRaw } = useReadContract({
    ...presaleContract,
    functionName: 'rate',
    query: { enabled: !!presaleContract.address },
  });
  ...
  const { data: walletSldRaw } = useReadContract({
    ...tokenContract,
  ...
  const { data: presaleSldRaw } = useReadContract({
    ...tokenContract,
    functionName: 'balanceOf',
    args: [presaleContract.address],
  });
  ...
  const [bnbPriceUSD, setBnbPriceUSD] = useState<number | null>(null);
  const [tokenPreview, setTokenPreview] = useState<number>(0);
  const [usdPreview, setUsdPreview] = useState<number>(0);
  const [inputError, setInputError] = useState<string>('');

  const [walletSldBalance, setWalletSldBalance] = useState<number | null>(null);
  const [saleSldBalance, setSaleSldBalance] = useState<number | null>(null);

  const [purchases, setPurchases] = useState<
    { hash: string; bnb: number; sld: number | null; time: string }[]
  >([]);
  ...
  // Static config
  // ------------------------------
  const saleAddress = presaleContract.address;
  ...
  const texts = {
    ...
    fr: {
      ...
      purchaseSuccess: 'Achat réussi ! SLD envoyé dans votre portefeuille.',
      trustDesktopWarning:
  ...
  // ------------------------------
  // Derived values from on-chain reads
  // ------------------------------
  const rate = rateRaw ? Number(rateRaw) : null;
  const humanRate = rate ? rate / 1e18 : null; // 23,000,000 SLD / BNB (human)
  const saleActive = !!saleActiveRaw;

  const walletBnbBalance = bnbBalData
  ? Number(formatEther(bnbBalData.value))
  : null;

  useEffect(() => {
    if (walletSldRaw != null) {
      setWalletSldBalance(Number(walletSldRaw) / 10 ** tokenDecimals);
    }
  }, [walletSldRaw]);

  useEffect(() => {
    if (presaleSldRaw != null) {
      setSaleSldBalance(Number(presaleSldRaw) / 10 ** tokenDecimals);
    }
  }, [presaleSldRaw]);
  ...
  // ------------------------------
  // Environment detection (mobile / Trust Wallet)
  // ------------------------------
  useEffect(() => {
    ...
    // best-effort Trust Wallet detection
    const eth: any = (window as any).ethereum;
    if (eth) {
      setIsTrustWallet(
        !!eth.isTrustWallet ||
          (eth.isMetaMask && eth.isTrust && !eth.isBraveWallet)
      );
    }
  }, []);

  // ------------------------------
  // Referral parsing (?ref=0x...)
  // ------------------------------
  useEffect(() => {
    if (typeof window === 'undefined') return;
...
async function fetchPrice() {
  try {
    ...
    setBnbPriceUSD(data?.binancecoin?.usd || null);
  } catch (err) {
  ...
  // Input + preview
  // ------------------------------
  function updateTokenPreview(amount: string) {
    setBnbAmount(amount);
  ...
  // Set Max BNB (90% of wallet, capped by MAX_BNB)
  function setMaxBnb() {
  ...
  // ------------------------------
  // Buy tokens via Wagmi
  // ------------------------------
  const wrongNetwork = chainId !== undefined && chainId !== 56;
  ...
  async function handleBuy() {
    try {
      ...
      writeContract({
        ...
        value: parseEther(bnbAmount as `${number}`),
      });
    } catch (err) {
      console.error(err);
  ...
  }, [isPending, confirming, txSuccess, t.purchaseSuccess]);

  // Push into local purchase history when tx confirmed
  useEffect(() => {
  ...
  }, [txSuccess, txHash, humanRate, bnbAmount]);
  ...
  // Leaderboard (top 3 by SLD bought this session)
  ...
  return (
    ...
    >
          ...
          {isConnected && address ? (
            <div className="flex flex-col items-end gap-1">
              <div className="flex items-center gap-2 text-xs bg-gray-800/70 px-3 py-1.5 rounded-xl border border-gray-700">
                ...
                <span className="text-gray-200">{shortAddress(address)}</span>
              </div>
              <button
      ...
      <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        {/* Left: buy panel */}
        <div className={`${cardBg} p-6 rounded-2xl shadow-xl border ${cardBorder}`}>
          {isConnected && address ? (
            <>
              {/* Input */}
              <div className="mb-4">
        ...
        <div className="space-y-4">
          {/* Wallet overview */}
          <div className={`${cardBg} p-4 rounded-2xl border ${cardBorder}`}>
